...
def db_with_market(tmp_path: Path) -> Path:
    ...
    con.execute(
        ...
        (
            "m1", "c1", "test-slug", "Will X happen?",
            "tok_yes", "tok_no", 0.55, 5000.0, 1000.0,
            0.53, 100.0, 0.57, 100.0,
            1800000000000, 1, 0, 0, None,
            1700000000000, 1, 0, None,
        ),
    )
    con.execute(
...
def db_with_neg_risk_group(tmp_path: Path) -> Path:
    ...
    for i, (mid, slug, q) in enumerate([
        ("m_a", "slug-a", "QA"), ("m_b", "slug-b", "QB"), ("m_c", "slug-c", "QC"),
    ]):
        con.execute(
            "INSERT INTO markets VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (mid, "c_neg", slug, q, "ty", "tn", 0.50, 1000.0, 100.0,
             0.48, 10.0, 0.52, 10.0,
             1800000000000, 1, 0, 1, "neg_grp_1",
             1700000000000, 1, 0, None),
        )
    con.commit()
    con.close()
    ...
    for i in range(6):
        ...
        pq.write_table(
            pa.table({
                ...
                "slug": ["test-slug"], "question": ["Q"],
                "yes_token_id": [None], "no_token_id": [None],
                "mid_price": [0.50 + i * 0.01],
                "liquidity_usd": [1000.0], "volume_usd": [100.0],
                "best_bid_price": [0.48], "best_bid_size": [10.0],
                "best_ask_price": [0.52], "best_ask_size": [10.0],
                "end_time_ms": [1800000000000], "active": [True],
                "closed": [False], "neg_risk": [False],
                "neg_risk_market_id": [None], "fetched_at_ms": [1700000000000],
                "snapshot_taken_at_ms": [1700000000000 + i * 1000],
                "snapshot_id": [i + 1], "incomplete": [False],
                "event_id": [None], "category": ["test"],
            }),
            d / "00-00-00.parquet",
        )
    return root
