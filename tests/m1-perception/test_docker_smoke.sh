...
sleep 12
# Use -sS (no -f) because /health returns HTTP 503 when status=fail (no snapshots
# yet = expected on fresh deploy). We validate the JSON body, not the HTTP code.
curl -sS http://127.0.0.1:8088/health | python -c "import json,sys; d=json.load(sys.stdin); assert d['status'] in ('pass','warn','fail'), f'bad status: {d}'; print('health OK', d['status'])" || { echo "FAIL: /health" >&2; exit 1; }
