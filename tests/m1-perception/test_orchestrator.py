...
async def test_phase_timing_lines_emitted(tmp_path: Path) -> None:
    ...
    # tagged with its phase label (e.g. '1/8', '4/8'). Phase 1.1 plan-02
    # added phase 8/8 (translation sidecar) — fail-soft path expected here
    # because the test env has no TRANSLATION_* vars (ValidationError → reason
    # records 'config_invalid' but is_valid is unchanged and the phase banner
    # still emits start + done lines).
    for phase_num in range(1, 9):
        starts = [m for m in captured if f"Phase {phase_num}/8" in m and "start" in m]
        dones = [m for m in captured if f"Phase {phase_num}/8" in m and "done in" in m]
        assert len(starts) == 1, (
            f"phase {phase_num}/8 missing 'start' (or duplicated): {[m for m in captured if f'Phase {phase_num}/8' in m]}"
        )
        assert len(dones) == 1, (
            f"phase {phase_num}/8 missing 'done in' (or duplicated): {[m for m in captured if f'Phase {phase_num}/8' in m]}"
        )
...
# ─────────────────────────────────────────────────────────────────────────────
# Phase 1.1 plan-02 — Step 8 translation sidecar (two-path error handling)
# ─────────────────────────────────────────────────────────────────────────────


@pytest.mark.asyncio
async def test_step_8_translation_phase_present(tmp_path: Path) -> None:
    """The 8/8 banner must be emitted by the orchestrator.

    Test env has no TRANSLATION_* vars → ValidationError path → reason
    'config_invalid' is recorded but the phase banner still goes out.
    """
    from loguru import logger
    ...
    # Banner emitted
    starts = [m for m in captured if "Phase 8/8" in m and "start" in m]
    dones = [m for m in captured if "Phase 8/8" in m and "done in" in m]
    assert len(starts) == 1
    assert len(dones) == 1

    # Sidecar semantics: translation failure NEVER flips is_valid
    assert result.is_valid is True


@pytest.mark.asyncio
async def test_step_8_config_validation_error_does_not_flip_is_valid(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """When .env is missing → TranslationConfig() ValidationError → reason
    records 'config_invalid' but is_valid stays True."""
    # Strip any host TRANSLATION_* env so ValidationError fires reliably.
    ...
    assert result.is_valid is True
    assert result.translation_skipped_reason == "config_invalid"


@pytest.mark.asyncio
async def test_step_8_config_error_records_reason(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """ConfigError raised mid-flight by translate_pending → reason
    'config_invalid', is_valid still True."""
    # Provide enough env so TranslationConfig() loads OK, then make the
    # actual translate_pending call raise ConfigError.
    monkeypatch.setenv("TRANSLATION_API_BASE", "https://example.com/v1")
    ...
    from polyarb.translation.translator import ConfigError

    async def fake_translate_pending(*args, **kwargs):
        raise ConfigError("simulated bad api_key")

    with patch(
        "polyarb.snapshot.orchestrator.GammaClient", return_value=fake_gamma
    ), patch("polyarb.snapshot.orchestrator.ClobReaderClient") as ClobMock, patch(
        "polyarb.translation.translator.translate_pending",
        side_effect=fake_translate_pending,
    ):
        clob_inst = ClobMock.return_value
        clob_inst.get_books = AsyncMock(return_value=_books_as_objects(clob_data["books"]))
        clob_inst.get_prices_buy_sell = AsyncMock(
...
@pytest.mark.asyncio
async def test_step_8_transient_error_records_reason(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """TransientError → reason 'transient_or_unknown', is_valid still True,
    ...
    from polyarb.translation.translator import TransientError

    async def fake_translate_pending(*args, **kwargs):
        raise TransientError("simulated network blip")
    ...
    assert result.is_valid is True
    assert result.translation_skipped_reason == "transient_or_unknown"


@pytest.mark.asyncio
async def test_step_8_success_logs_summary(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Successful translate_pending → INFO log includes the summary line +
    translation_skipped_reason is None."""
    from loguru import logger
    ...
    from polyarb.translation.translator import TranslateSummary

    async def fake_translate_pending(*args, **kwargs):
        return TranslateSummary(translated=5, skipped=0, dead=0, total_tokens=420)
    ...
    assert result.is_valid is True
    assert result.translation_skipped_reason is None
    summary_lines = [m for m in captured if "+5 translated" in m]
    assert len(summary_lines) == 1, f"missing summary line: {captured}"
