...
def test_integrity_invalidates_but_expired_lease_pauses_same_epoch() -> None:
    policy = _policy()
    paused = policy.apply(
        _state(policy),
    ...
    assert paused.pending_recovery_started is True
    assert paused.coverage_seconds == 0

    invalidated = policy.apply(
        _state(policy),
        _fact("integrity", NOW + timedelta(hours=1), reason="integrity.conflict"),
    )
    assert invalidated.state is QualificationState.INVALIDATED
    assert invalidated.invalidated_at == NOW + timedelta(hours=1)
    assert invalidated.invalidation_reason == "integrity.conflict"
...
def test_gap_equal_to_limit_is_allowed_but_gap_over_limit_pauses() -> None:
    ...
    assert broken.state is QualificationState.ACCUMULATING
    assert broken.pending_recovery_started is True
    assert broken.coverage_seconds == 60
...
def test_contained_recovery_clears_pending_start_and_allows_qualification() -> None:
    ...
    assert qualified.qualified_at == NOW + timedelta(seconds=62)
...
@pytest.mark.parametrize("reason", sorted(BLOCKING_REASONS))
def test_blocking_reason_matrix_pauses_same_epoch(reason: str) -> None:
    policy = _policy()
    result = policy.apply(_state(policy), _fact("block", NOW, reason=reason))
    assert result.state is QualificationState.ACCUMULATING
    assert result.pending_recovery_started is True
    assert result.invalidated_at is None


def test_unresolved_p1_and_three_freshness_classes_pause_without_reset() -> None:
    policy = _policy()
...
def test_contained_process_replacement_must_finish_within_slo() -> None:
    ...
    assert broken.state is QualificationState.ACCUMULATING
    assert broken.pending_recovery_started is True
    assert broken.invalidated_at is None


def test_identity_drift_and_explicit_scoped_progress_regression_break() -> None:
    policy = _policy()
...
def test_unscoped_cross_source_counts_do_not_create_false_regression() -> None:
    policy = _policy(max_gap_seconds=60)
    state = policy.apply(_state(policy), _fact("quote-count", NOW, progress_count=10))

    next_product = policy.apply(
        state,
        _fact("structure-count", NOW + timedelta(seconds=1), progress_count=3),
    )

    assert next_product.state is QualificationState.ACCUMULATING
    assert next_product.invalidation_reason is None
...
def test_decision_rejects_impossible_four_state_combinations() -> None:
    ...
    with pytest.raises(ValueError, match="only invalidated"):
        replace(initial, invalidated_at=NOW, invalidation_reason="integrity.conflict")
