...
These tests spin a real Postgres 16 via testcontainers, run `alembic upgrade head`
...
def test_003_creates_all_tables(pg_dsn):
    """After upgrade head, 5 l2_* tables must exist."""
    r = _run_alembic(pg_dsn, "upgrade head")
    assert r.returncode == 0, f"alembic upgrade failed:\nSTDOUT={r.stdout}\nSTDERR={r.stderr}"
    rows = _q(pg_dsn, "SELECT tablename FROM pg_tables WHERE schemaname='public' ORDER BY tablename")
    names = {row["tablename"] for row in rows}
    for expected in ("l2_candidates", "l2_top_of_book", "l2_trades", "l2_signals", "l2_event_cursor"):
...
def test_003_rls_enabled(pg_dsn):
    ...
    _run_alembic(pg_dsn, "upgrade head")
    rows = _q(
        pg_dsn,
...
def test_idempotent_replay(pg_dsn):
    """upgrade head → downgrade base → upgrade head must succeed end-to-end."""
