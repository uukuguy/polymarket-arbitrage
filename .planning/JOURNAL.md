...
- [SECURITY] The separate `m1_runtime_event_writer` role credential was rotated after an old SQL-editor value was discovered. Its writer Machine now uses the project-qualified IPv4 pooler login through a versioned DSN mapping; health plus an authenticated idempotent database probe passed without creating a new incident.
...
- [ACCEPTANCE] A bounded fail-closed self-check at 626 seconds passed from four persisted samples with all three fixed worker identities started, no new expired leases/circuits, and successful jobs advancing to 1,577. This is early evidence only; the verifier retains its required 86,400-second production threshold.
- [OPERABILITY] `make control-plane-cloud-soak-verify run_id=<run>` is now the uniform read-only entrypoint for immutable Supabase evidence. It was exercised at 928 seconds / five samples with a PASS; the default 86,400-second threshold remains unchanged.
...
- [PREPARED / DEFERRED] Watchdog now accepts repeatable exact cross-app `--secondary-target <app>/<machine>` targets, allowing the isolated Dashboard ledger writer to be monitored together with the evidence sampler. Focused CLI/watchdog tests pass. Deployment is deliberately deferred until the active immutable 24-hour run closes, because changing the watchdog topology would invalidate that run.
