...
const eventHref = (eventId: unknown) => `/business/events/${encodeURIComponent(String(eventId))}?from=structure`;

function EventLink({ event }: { event: StructureIntelligenceItem }) { return <a href={eventHref(event.event_id)} style={{ color: "#c8dcff" }}>{text(event.title)}</a>; }
