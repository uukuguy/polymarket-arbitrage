...
export default async function EventResearchPage({ params, searchParams }: {
  ...
  const groupCardStyle = (group: EventResearchGroup) => ({
    borderTop: "1px solid #303030",
    padding: "12px 0",
    ...(focus?.group_id === group.group_id ? { background: "#172236", outline: "2px solid #8ab4f8", outlineOffset: 3 } : {}),
  });
  const coverageDefectRank = (group: EventResearchGroup) => {
    switch (record(group.quote_coverage).coverage_state) {
      case "incomplete-missing": return 0;
      case "complete-non-executable": return 1;
      case "unknown": return 2;
      default: return 3;
    }
  };
  const coverageGroups = [...groups].sort((left, right) => coverageDefectRank(left) - coverageDefectRank(right) || left.group_id.localeCompare(right.group_id));
