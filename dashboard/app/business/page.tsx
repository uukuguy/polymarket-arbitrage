...
export default async function BusinessPage() {
  ...
  return <BusinessShell overview={data} title="Business research" subtitle="A lineage-consistent view of what M1 has published, what is ready to use, and what is still not observable.">
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(255px, 1fr))", gap: 12 }}>
      ...
      <ProductCard title="Analysis funnel" item={data.analysis}><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginTop: 14 }}><Metric label="Structure → Quotes" value={`${(data.analysis.component_counts?.structure_records ?? 0).toLocaleString()} → ${(data.analysis.component_counts?.quote_records ?? 0).toLocaleString()}`} note="lineage-bound research coverage" /><Metric label="Certified" value={data.analysis.component_counts?.certified_opportunities?.toLocaleString() ?? "—"} note="only same-generation results" /></div><p><a href="/business/analysis">Inspect analysis truth →</a></p></ProductCard>
