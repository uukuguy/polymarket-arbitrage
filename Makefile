...
.PHONY: supabase-migrate supabase-migrate-test control-plane-migrate-test control-plane-preflight control-plane-db-role-preflight control-plane-db-role-provision control-plane-db-role-verify control-plane-db-role-disable control-plane-fly-topology-audit control-plane-egress-preflight control-plane-render-rollout control-plane-render-machine-update control-plane-render-runtime-maintenance control-plane-render-runtime-restore control-plane-verify-runtime-maintenance-outcome control-plane-verify-machine-update control-plane-verify-shadow-parity control-plane-verify-fault-soak control-plane-soak-start control-plane-soak-sample control-plane-soak-verify control-plane-cloud-soak-start control-plane-cloud-soak-verify control-plane-api-serve control-plane-runtime-event-writer-serve control-plane-shadow-sync control-plane-status control-plane-capacity control-plane-business-brief control-plane-structure-index-backfill control-plane-structure-intelligence-backfill control-plane-analysis-candidate-backfill quote-control-plane-once quote-refresh-admit-once structure-control-plane-once structure-control-plane-source-once structure-control-plane-shadow-once structure-control-plane-shadow-publish control-plane-tick-once control-plane-serve control-plane-serve-coordinator control-plane-serve-structure-range control-plane-serve-quote-batch control-plane-alert-serve control-plane-watchdog-serve control-plane-watchdog-verify control-plane-watchdog-supervisor-deploy control-plane-watchdog-supervisor-verify runtime-policy-replay runtime-fault-matrix m1-production-commissioning-plan m1-production-commissioning-complete m1-production-commissioning-stale-owner m1-production-commissioning-progress-stall m1-production-commissioning-retry-budget m1-production-commissioning-heartbeat-outage m1-production-commissioning-worker-exit m1-production-commissioning-source-receipt-gap m1-production-commissioning-quote-batch-incomplete m1-production-commissioning-normalization-payload-corrupt m1-production-commissioning-structure-parity-mismatch m1-production-commissioning-publication-pointer-conflict m1-production-commissioning-r2-read-timeout m1-production-commissioning-r2-write-timeout m1-production-commissioning-stale-quote-pointer m1-production-commissioning-clob-missing-leg m1-production-commissioning-gamma-timeout m1-production-commissioning-gamma-malformed-page m1-production-commissioning-assemble m1-production-commissioning-verify runtime-controller-status runtime-observe-verify runtime-reconcile-once runtime-reconcile-serve supabase-reconcile r2-list
...
## control-plane-render-rollout: Render local-only seven-app runtime/qualification/alert-delivery topology and checklist; usage enable=1 release_id=<40-char-lowercase-git-sha>; never contacts cloud resources.
control-plane-render-rollout:
	@test "$(enable)" = "1" || (echo "usage: make control-plane-render-rollout enable=1 api_app=<app> worker_app=<app> alert_app=<app> alert_delivery_app=<app> runtime_event_writer_app=<app> runtime_controller_app=<app> qualification_worker_app=<app> release_id=<40-char-lowercase-git-sha> expected_database=<name> output_dir=<empty-dir> [runtime_recovery_allowed_targets='app/id ...']" >&2; exit 2)
	@test -n "$(api_app)" -a -n "$(worker_app)" -a -n "$(alert_app)" -a -n "$(alert_delivery_app)" -a -n "$(runtime_event_writer_app)" -a -n "$(runtime_controller_app)" -a -n "$(qualification_worker_app)" -a -n "$(release_id)" -a -n "$(expected_database)" -a -n "$(output_dir)" || (echo "all seven app identities, release_id=<40-char-lowercase-git-sha>, expected_database and output_dir are required" >&2; exit 2)
	@printf '%s\n' "$(release_id)" | grep -Eq '^[0-9a-f]{40}$$' || (echo "release_id=<40-char-lowercase-git-sha> is required" >&2; exit 2)
	@uv run python -m polyarb.cli_control_plane render-rollout --enable --api-app "$(api_app)" --worker-app "$(worker_app)" --alert-app "$(alert_app)" --alert-delivery-app "$(alert_delivery_app)" --runtime-event-writer-app "$(runtime_event_writer_app)" --runtime-controller-app "$(runtime_controller_app)" --qualification-worker-app "$(qualification_worker_app)" --release-id "$(release_id)" $(foreach target,$(runtime_recovery_allowed_targets),--runtime-recovery-allowed-target "$(target)") --expected-database "$(expected_database)" --output-dir "$(output_dir)" --json

## control-plane-render-machine-update: Translate one fresh Machine GET plus rendered Fly TOML into a full optimistic Machines API payload; optional guest shape requires all three target_* fields; local-only and redacted.
control-plane-render-machine-update:
	@test -n "$(current_machine)" -a -n "$(fly_config)" -a -n "$(expected_app)" -a -n "$(machine_id)" -a -n "$(target_image)" -a -n "$(output)" || (echo "usage: make control-plane-render-machine-update current_machine=<fresh-machine.json> fly_config=<rendered-fly.toml> expected_app=<app> machine_id=<id> target_image=<image> output=<new-update.json> [update_env_from_fly=POLYARB_QUALIFICATION_RELEASE_ID] [target_cpu_kind=shared target_cpus=2 target_memory_mb=1024]" >&2; exit 2)
	@uv run python -m polyarb.control_plane.fly_machine_update render --current-machine "$(current_machine)" --fly-config "$(fly_config)" --expected-app "$(expected_app)" --expected-machine-id "$(machine_id)" --target-image "$(target_image)" $(foreach key,$(update_env_from_fly),--update-env-from-fly "$(key)") $(if $(target_cpu_kind),--target-cpu-kind "$(target_cpu_kind)") $(if $(target_cpus),--target-cpus "$(target_cpus)") $(if $(target_memory_mb),--target-memory-mb "$(target_memory_mb)") --output "$(output)" --json

## control-plane-render-runtime-maintenance: Replace the resident controller with one exact recovery process; never SSH a second Python into the 256MB Machine.
control-plane-render-runtime-maintenance:
	@test -n "$(current_machine)" -a -n "$(expected_app)" -a -n "$(machine_id)" -a -n "$(target_type)" -a -n "$(target_id)" -a -n "$(expected_action)" -a -n "$(controller_id)" -a -n "$(output)" || (echo "usage: make control-plane-render-runtime-maintenance current_machine=<fresh-machine.json> expected_app=<controller-app> machine_id=<id> target_type=circuit target_id=<exact-job-key> expected_action=probe-circuit controller_id=<unique-maintenance-id> output=<maintenance-update.json>" >&2; exit 2)
	@uv run python -m polyarb.control_plane.fly_machine_update render-runtime-maintenance --current-machine "$(current_machine)" --expected-app "$(expected_app)" --expected-machine-id "$(machine_id)" --target-type "$(target_type)" --target-id "$(target_id)" --expected-action "$(expected_action)" --controller-id "$(controller_id)" --output "$(output)" --json

## control-plane-render-runtime-restore: Restore the exact saved observe-only controller after a process-replacement maintenance turn.
control-plane-render-runtime-restore:
	@test -n "$(baseline_machine)" -a -n "$(maintenance_machine)" -a -n "$(maintenance_payload)" -a -n "$(machine_id)" -a -n "$(output)" || (echo "usage: make control-plane-render-runtime-restore baseline_machine=<saved-baseline.json> maintenance_machine=<fresh-machine.json> maintenance_payload=<maintenance-update.json> machine_id=<id> output=<restore-update.json>" >&2; exit 2)
	@uv run python -m polyarb.control_plane.fly_machine_update render-runtime-restore --baseline-machine "$(baseline_machine)" --maintenance-machine "$(maintenance_machine)" --maintenance-payload "$(maintenance_payload)" --expected-machine-id "$(machine_id)" --output "$(output)" --json

## control-plane-verify-runtime-maintenance-outcome: Prove a maintenance turn created one new succeeded exact action; stopped process state is insufficient.
control-plane-verify-runtime-maintenance-outcome:
	@test -n "$(before_status)" -a -n "$(after_status)" -a -n "$(maintenance_payload)" || (echo "usage: make control-plane-verify-runtime-maintenance-outcome before_status=<before-control-plane.json> after_status=<after-control-plane.json> maintenance_payload=<maintenance-update.json>" >&2; exit 2)
	@uv run python -m polyarb.control_plane.fly_machine_update verify-runtime-maintenance-outcome --before-status "$(before_status)" --after-status "$(after_status)" --maintenance-payload "$(maintenance_payload)" --json
...
control-plane-verify-machine-update:
	@test -n "$(updated_machine)" -a -n "$(update_payload)" -a -n "$(machine_id)" -a -n "$(region)" || (echo "usage: make control-plane-verify-machine-update updated_machine=<fresh-machine.json> update_payload=<rendered-update.json> machine_id=<id> region=<region>" >&2; exit 2)
	@uv run python -m polyarb.control_plane.fly_machine_update verify --updated-machine "$(updated_machine)" --update-payload "$(update_payload)" --expected-machine-id "$(machine_id)" --expected-region "$(region)" --json
...
## control-plane-cloud-soak-start: Atomically create a new formal cloud-soak baseline; requires run_id=, explicit scoped DSN and Fly read token.
...
## control-plane-capacity: Read only M1 database size, budget state, and largest relations.
control-plane-capacity:
	@set -a; [ -f .env ] && . ./.env; set +a; \
	if [ -z "$$POLYARB_SUPABASE_DB_DSN" ]; then echo "ERROR: POLYARB_SUPABASE_DB_DSN is required" >&2; exit 2; fi; \
	uv run python -m polyarb.cli_control_plane capacity --json

## control-plane-structure-index-backfill: Rebuild compact current Structure rows from authenticated R2 artifacts; requires enable=1 and generation_key=.
control-plane-structure-index-backfill:
	@test "$(enable)" = "1" -a -n "$(generation_key)" || (echo "usage: make control-plane-structure-index-backfill enable=1 generation_key=<published-structure-generation>" >&2; exit 2)
	@set -a; [ -f .env ] && . ./.env; set +a; \
	if [ -z "$$POLYARB_CONTROL_PLANE_DB_ADMIN_DSN" ]; then echo "ERROR: POLYARB_CONTROL_PLANE_DB_ADMIN_DSN is required for one-time index retirement" >&2; exit 2; fi; \
	POLYARB_SUPABASE_DB_DSN="$$POLYARB_CONTROL_PLANE_DB_ADMIN_DSN" uv run python -m polyarb.cli_control_plane structure-index-backfill --enable --generation-key "$(generation_key)" --json

## control-plane-structure-intelligence-backfill: Build bounded Structure event/risk research; requires enable=1 and generation_key=.
control-plane-structure-intelligence-backfill:
	@test "$(enable)" = "1" -a -n "$(generation_key)" || (echo "usage: make control-plane-structure-intelligence-backfill enable=1 generation_key=<published-structure-generation>" >&2; exit 2)
	@set -a; [ -f .env ] && . ./.env; set +a; \
	if [ -z "$$POLYARB_CONTROL_PLANE_DB_ADMIN_DSN" ]; then echo "ERROR: POLYARB_CONTROL_PLANE_DB_ADMIN_DSN is required for intelligence materialization" >&2; exit 2; fi; \
	POLYARB_SUPABASE_DB_DSN="$$POLYARB_CONTROL_PLANE_DB_ADMIN_DSN" uv run python -m polyarb.cli_control_plane structure-intelligence-backfill --enable --generation-key "$(generation_key)" --json

## control-plane-analysis-candidate-backfill: Persist compact current group candidates/rejections; requires enable=1 and generation_key=.
control-plane-analysis-candidate-backfill:
	@test "$(enable)" = "1" -a -n "$(generation_key)" || (echo "usage: make control-plane-analysis-candidate-backfill enable=1 generation_key=<current-quote-generation>" >&2; exit 2)
	@set -a; [ -f .env ] && . ./.env; set +a; \
	if [ -z "$$POLYARB_CONTROL_PLANE_DB_ADMIN_DSN" ]; then echo "ERROR: POLYARB_CONTROL_PLANE_DB_ADMIN_DSN is required for bounded candidate materialization" >&2; exit 2; fi; \
	POLYARB_SUPABASE_DB_DSN="$$POLYARB_CONTROL_PLANE_DB_ADMIN_DSN" uv run python -m polyarb.cli_control_plane analysis-candidate-backfill --enable --generation-key "$(generation_key)" --json

## control-plane-business-brief: Read a one-screen M1 business brief; optional format=text|json.
control-plane-business-brief: export CONTROL_PLANE_BUSINESS_BRIEF_FORMAT := $(value format)
...
## m1-production-commissioning-complete: Resume all 8 normal turns, 66 exact-image attack proofs, real E2E lineage, and assemble one verified envelope; isolated migrated loopback databases only.
...
m1-production-commissioning-stale-owner:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-stale-owner evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness stale-owner --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-progress-stall: Prove all eight live-lease progress-stall recoveries in isolated migrated loopback databases; never contacts production.
m1-production-commissioning-progress-stall:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-progress-stall evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness progress-stall --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-retry-budget: Prove all eight retry-budget circuit recoveries in isolated migrated loopback databases; never contacts production.
m1-production-commissioning-retry-budget:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-retry-budget evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness retry-budget --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-heartbeat-outage: Prove all eight live-attempt heartbeat recoveries in isolated migrated loopback databases; never contacts production.
m1-production-commissioning-heartbeat-outage:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-heartbeat-outage evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness heartbeat-outage --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-worker-exit: Prove all eight expired-worker reclaim recoveries in isolated migrated loopback databases; never contacts production.
m1-production-commissioning-worker-exit:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-worker-exit evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness worker-exit --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-source-receipt-gap: Prove the Structure source receipt barrier and single materializer release in an isolated migrated loopback database; never contacts production.
m1-production-commissioning-source-receipt-gap:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-source-receipt-gap evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness source-receipt-gap --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-quote-batch-incomplete: Prove the Quote batch barrier, durable producer incident, fenced retry, and complete pointer publication in an isolated migrated loopback database; never contacts production.
m1-production-commissioning-quote-batch-incomplete:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-quote-batch-incomplete evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness quote-batch-incomplete --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-quote-admission-missing-shard: Prove safe exact diagnosis and policy-due recovery of one missing Structure shard in Quote admission using an isolated migrated loopback database; never contacts production.
m1-production-commissioning-quote-admission-missing-shard:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-quote-admission-missing-shard evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness quote-admission-missing-shard --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-normalization-payload-corrupt: Prove an authenticated schema-invalid Structure shard is visibly quarantined while the prior certified pointer remains authoritative; isolated migrated loopback database only.
m1-production-commissioning-normalization-payload-corrupt:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-normalization-payload-corrupt evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness normalization-payload-corrupt --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-structure-parity-mismatch: Prove a frozen Structure count conflict invalidates qualification while the prior certified pointer remains authoritative; isolated migrated loopback database only.
m1-production-commissioning-structure-parity-mismatch:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-structure-parity-mismatch evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness structure-parity-mismatch --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-publication-pointer-conflict: Prove stale Structure, Quote, and Opportunity publishers cannot move a newer current pointer; isolated migrated loopback databases only.
m1-production-commissioning-publication-pointer-conflict:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-publication-pointer-conflict evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness publication-pointer-conflict --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-r2-read-timeout: Prove all six R2-reading production nodes retry the same immutable artifact and input identity after a typed GET timeout; isolated migrated loopback databases only.
m1-production-commissioning-r2-read-timeout:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-r2-read-timeout evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness r2-read-timeout --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-r2-write-timeout: Prove all seven R2-writing production nodes HEAD-verify an already-committed immutable object after PUT response timeout before committing one receipt; isolated migrated loopback databases only.
m1-production-commissioning-r2-write-timeout:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-r2-write-timeout evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness r2-write-timeout --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-stale-quote-pointer: Prove stale Quote authority blocks Opportunity publication and fresh Structure/Quote lineage resolves the blocking incident; isolated migrated loopback database only.
m1-production-commissioning-stale-quote-pointer:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-stale-quote-pointer evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
...
## m1-production-commissioning-clob-missing-leg: Prove omitted CLOB coverage cannot publish a Quote batch; missing-book evidence is terminal without retry/circuit; isolated migrated loopback database only.
m1-production-commissioning-clob-missing-leg:
	...
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness clob-missing-leg --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-clob-429: Prove a body-free typed CLOB 429 records durable retry state and recovers one Quote batch; isolated migrated loopback database only.
m1-production-commissioning-clob-429:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-clob-429 evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness clob-429 --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-gamma-timeout: Prove one Gamma timeout resets transport and recovers the exact Structure source page; isolated migrated loopback database only.
m1-production-commissioning-gamma-timeout:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-gamma-timeout evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness gamma-timeout --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json

## m1-production-commissioning-gamma-malformed-page: Prove a body-free malformed Gamma page is rejected and the exact Structure source page recovers; isolated migrated loopback database only.
m1-production-commissioning-gamma-malformed-page:
	@test -n "$$POLYARB_CONTROL_PLANE_TEST_DSN" || (echo "ERROR: explicitly export POLYARB_CONTROL_PLANE_TEST_DSN" >&2; exit 2)
	@test -n "$(evidence_root)" || (echo "usage: make m1-production-commissioning-gamma-malformed-page evidence_root=<dir> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning_harness gamma-malformed-page --root "$(evidence_root)" --release-id "$(expected_release)" --config-id "$(expected_config)" --json
...
m1-production-commissioning-verify:
	@test -n "$(evidence)" || (echo "usage: make m1-production-commissioning-verify evidence=<json> expected_release=<40-char-sha> expected_config=<sha256:id>" >&2; exit 2)
	@test -n "$(expected_release)" || (echo "ERROR: expected_release is required" >&2; exit 2)
	@test -n "$(expected_config)" || (echo "ERROR: expected_config is required" >&2; exit 2)
	@uv run python -m polyarb.control_plane.production_commissioning verify --evidence "$(evidence)" --expected-release "$(expected_release)" --expected-config "$(expected_config)" --json
...
## runtime-reconcile-until: Boundedly wait for one exact busy recovery lane, then execute once; maintenance use only.
runtime-reconcile-until:
	@test "$(enable)" = "1" -a -n "$(target_type)" -a -n "$(target_id)" -a -n "$(expected_action)" -a -n "$(controller_id)" -a -n "$(owner_id)" || (echo "usage: make runtime-reconcile-until enable=1 target_type=circuit target_id=<exact-job-key> expected_action=probe-circuit controller_id=<unique-id> owner_id=<unique-id>" >&2; exit 2)
	@uv run python -m polyarb.cli_control_plane runtime-reconcile-until --enable --controller-id "$(controller_id)" --owner-id "$(owner_id)" --worker-id "$(or $(worker_id),runtime-recovery-executor)" --target-type "$(target_type)" --target-id "$(target_id)" --expected-action "$(expected_action)" --max-wait-seconds "$(or $(max_wait_seconds),45)" --retry-interval-seconds "$(or $(retry_interval_seconds),1)" --json
...
.PHONY: runtime-policy-replay runtime-fault-matrix runtime-controller-status runtime-reconcile-once runtime-reconcile-until runtime-reconcile-serve qualification-status qualification-certificates qualification-serve
...
## quote-refresh-admit-once: Admit at most one due 300-second recurring Quote run from durable lineage; requires enable=1 and DSN. Performs no provider or R2 I/O.
quote-refresh-admit-once:
	@test "$(enable)" = "1" || (echo "usage: make quote-refresh-admit-once enable=1" >&2; exit 2)
	@set -a; [ -f .env ] && . ./.env; set +a; \
	if [ -z "$$POLYARB_SUPABASE_DB_DSN" ]; then echo "ERROR: POLYARB_SUPABASE_DB_DSN is required" >&2; exit 2; fi; \
	uv run python -m polyarb.cli_control_plane quote-refresh-admit-once --enable --json
...
## control-plane-egress-preflight: Verify a positive M1 byte budget and that no M1 deployment template reintroduces retired L2.
control-plane-egress-preflight:
	@uv run python -c "from polyarb.config import Settings; print({'daily_egress_budget_bytes': Settings().m1_daily_egress_budget_bytes})"
	@! rg -n 'polyarb-l2|l2_candidate_refresh' deploy/control-plane
