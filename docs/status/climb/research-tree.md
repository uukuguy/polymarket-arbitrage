...
- **H-054** [testing]: The transactional Quote worker must reject a CLOB response that omits any requested token before R2 or receipt publication, then recover the same immutable batch through the shared durable retry policy after complete coverage is restored.
