...
| Disposable stale-owner fence | `PreparedNormalTurn` captures each node's real terminal transaction; `claim_job` is the only lease takeover path; `StaleOwnerCommissioningAdapter` maps it to the runner lifecycle | virtual API time crosses the persisted lease boundary; no direct row mutation; old epoch executes the same terminal call and must raise `StaleLeaseError`; cleanup rechecks zero stale effects | original/replacement attempt IDs, zero old-epoch success facts, replacement success event and business postcondition in seven exclusive stage files plus proof | eight-node stale-owner real-PostgreSQL and cleanup-safe runner contracts |
