...
| 12-Research | [Event Research Workbench](12-event-research-workbench.md) | 如何从 Structure→Quote Coverage→Analysis 读一个 event；observed、executable、missing 与理论经济性各代表什么；为什么它不是下单授权 |
...
| 102 | [发布后关键路径决定资格新鲜度](102-发布后关键路径决定资格新鲜度.md) | 发布时间戳分解 / 904 秒真正关键路径 / Opportunity 有界并发 / 心跳与 drain |
| 103 | [满批消费者不能固定休眠](103-满批消费者不能固定休眠.md) | batch 是事务边界而非吞吐上限 / 满页立即续取 / 空闲才等待 / 256MB 观察纪律 |
| 104 | [新鲜度恢复必须匹配产品](104-新鲜度恢复必须匹配产品.md) | 产品作用域恢复 / 通用事件不能串扰 / 暂停墙钟不计 coverage / 重放一致性 |
| 105 | [市场全集与可执行报价必须使用两个时钟](105-市场全集与可执行报价必须使用两个时钟.md) | Structure/Quote 独立代次 / durable lost-wakeup / 过期 Quote 与下一代预案 |
| 106 | [M1 日常业务情报操作指南](106-M1日常业务情报操作指南.md) | 能区分业务可用、零认证机会、资格暂停和权威不可用，并作出对应日报结论 |
| 107 | [原子业务研究视图](107-原子业务研究视图.md) | 为什么业务页必须同快照、如何用 Structure→Quote→Opportunity lineage 判读、为什么 Analysis 不能伪造漏斗 |
