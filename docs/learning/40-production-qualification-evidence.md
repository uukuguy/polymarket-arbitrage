...
read-only baseline 能证明 HTTP、coverage、Quote age、policy、Reconciliation、
当前 open Incident，以及 Qualification 面显式统计的 cross-membership Quote 和
过期 collecting lease。但它不能凭一次 GET 证明：

- fault 注入到 detect 的 MTTD；
- detect 到 contained 的时长；
- fault 注入窗口内是否新产生这两类问题。

因此 fault adapter 要在同一个 release/machine/boot 上比较 pre/post 计数，并仍由
fault-specific durable evidence 提供 MTTD/containment。字段缺失会生成 `missing-*`
reason，生产 verdict 保持 FAIL。这是证据缺口，不是系统故障的虚构。
...
4. Qualification endpoint 不可用时，为什么 collector 不能沿用上一次的 0？
