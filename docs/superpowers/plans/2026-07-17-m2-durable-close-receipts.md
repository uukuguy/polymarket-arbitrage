...
- Modify: climb state/evidence files generated by `make climb-cycle hypothesis=H-002`
...
git add .planning/workstreams/m2-combinatorial/ROADMAP.md \
  ...
  .planning/JOURNAL.md .planning/climb
git commit -m "docs(m2): close durable close receipts phase"
```

If the climb state is stored outside `.planning/climb`, stage the exact tracked paths shown by `git status` instead; never use a broad add that captures unrelated user files.
