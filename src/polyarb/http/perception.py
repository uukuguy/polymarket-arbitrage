...
def _producer_progress(db_path: Path, quote_worker_runtime: Any | None = None) -> dict[str, Any]:
    ...
    quote_supervisor_receipt = perception_store.latest_producer_receipt("quote")
    structure_store = SQLiteStore(db_path)
    structure_attempt = structure_store.get_latest_snapshot_attempt()
    structure_sync_window = structure_store.get_latest_structure_sync()
    ...
    return {
        ...
        "quote": {
            ...
            "hydration": hydration,
            "supervisor_receipt": (
                None if quote_supervisor_receipt is None else asdict(quote_supervisor_receipt)
            ),
        },
        "structure": {
            "attempt": structure_attempt,
            "sync_window": structure_sync_window,
